...
const SITE_USER_KEY = "nexdomus_user";
...
function getUser() {
  const controlUser = parseJSON(localStorage.getItem(CONTROL_USER_KEY), null);
  if (controlUser) return controlUser;

  const siteUser = parseJSON(localStorage.getItem(SITE_USER_KEY), null);
  if (siteUser) {
    localStorage.setItem(CONTROL_USER_KEY, JSON.stringify(siteUser));
    return siteUser;
  }
